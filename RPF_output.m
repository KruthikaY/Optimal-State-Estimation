...
x_rpf = zeros(1, K_max + 1);
x_rpf(1) = sum(x_particles .* w_particles);

% Regularization bandwidth
h = 1.06 * std(x_particles) * N_particles^(-1/5);  % Silverman's rule
...
for k = 2:K_max + 1
    ...
    % 4. Resample with Regularization
    ...
    if Neff < N_particles / 2
        ...
        % Add jitter using Gaussian kernel (Regularization)
        x_particles = x_particles + h * randn(N_particles, 1);
    end

    % 5. Estimate State (weighted mean)
    x_rpf(k) = sum(x_particles .* w_particles);
end

% Plot Results
figure;
plot(0:K_max, x_true, 'k-', 'DisplayName', 'True x_k'); hold on;
plot(0:K_max, x_rpf, 'g--', 'DisplayName', 'Improved Particle Estimate');
xlabel('Time k'); ylabel('x_k');
legend; grid on;
title('Improved Particle Filter Estimate vs True State');

k_vals = (0:K_max)';
abs_error = abs(x_true - x_rpf);
pf_table = table(k_vals, x_true', x_rpf', abs_error', ...
    'VariableNames', {'k', 'x_true', 'x_pf_estimate', 'abs_error'});

disp('Improved Particle Filter Results:');
disp(pf_table(1:101, :));
